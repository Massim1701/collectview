...
| **Claude Code** | **Feedback-Mail gebaut** (02.09.2026, `5ac0103`): Rückmeldungen landeten bisher nur in der Tabelle `feedback`, niemand wurde benachrichtigt. Neue Edge Function `feedback-melden` schickt sie an die Support-Adresse — deployt, geprüft. **Ende zu Ende geprüft (02.09.2026)** mit einem Testnutzer, danach restlos aufgeräumt: Rückmeldung schreiben → Function → formsubmit. Dabei kam der eigentliche Fund: **die Tabelle `feedback` existierte gar nicht**, `db/feedback.sql` war nie gelaufen — die ganze Funktion war tot, jede Rückmeldung wäre abgeprallt. Jetzt angewandt. Zwei Fehler in der Function fielen dabei ebenfalls auf: sie schickte keinen `Referer` (formsubmit weist das ab) und prüfte nur den HTTP-Status — der ist bei formsubmit auch im Fehlerfall `200`, sie hätte jeden Fehlschlag als Erfolg verbucht. Beides behoben. **Es fehlt nur noch ein Klick:** die Aktivierungsmail von formsubmit in `supportcollectview.site@gmail.com` bestätigen. Danach läuft es ohne weitere Änderung — `FEEDBACK_MAIL_URL` ist gesetzt (serverseitig genügt die Adresse, der Hash ist nur für öffentliches HTML nötig). — **Passwort-vergessen entsperrt** (02.09.2026): `site_url` stand auf `http://localhost:3000`, die Allow-List war leer; ein Zurücksetzen hätte auf localhost gezeigt. Jetzt `https://collectview.site` plus `capacitor://localhost/**` (iOS), `https://localhost/**` (Android) und `http://localhost:*/**` (Entwicklung), gegengeprüft. — Sonst nichts offen. |
